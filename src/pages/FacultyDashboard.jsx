    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center">
      <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-sm space-y-5 text-center">
        ...
        <div className="flex gap-3 pt-2">
          <button onClick={onCancel} className="flex-1 py-2 border border-gray-200 rounded-lg text-sm font-medium text-gray-600 hover:border-[#8B0000] transition cursor-pointer">
            Cancel
          </button>
          <button onClick={onConfirm} className={`flex-1 py-2 rounded-lg text-sm font-medium text-white transition cursor-pointer ${confirmClass}`}>
            {confirmLabel}
          </button>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col gap-1">
        <div className="relative flex-1" style={{ height: 160 }}>
          {Y_LABELS.slice(0, -1).map((_, i) => (
            <div key={i} className="absolute left-0 right-0 border-t border-slate-100" style={{ top: `${(i / (Y_LABELS.length - 1)) * 100}%` }} />
          ))}
        ...
        <div className="space-y-3">
          {mockPendingFilms.map((film, idx) => (
            <div key={film.id} className="flex items-center gap-4 p-3 rounded-lg border border-slate-100 hover:border-slate-200 hover:bg-slate-50 transition">
              <div className="w-16 h-10 rounded-lg shrink-0 flex items-center justify-center" style={{ background: THUMBNAIL_COLORS[idx % THUMBNAIL_COLORS.length] }}>
    ...
    <div className="space-y-6">
        ...
        {filters.map((f) => (
          <button
            ...
            className={`px-4 py-2 rounded-full text-sm font-medium transition cursor-pointer ${
              activeFilter === f ? "bg-[#8B0000] text-white" : "bg-white border border-slate-200 text-slate-600 hover:border-[#8B0000]"
            }`}
          >
            {f}
          </button>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        {filtered.map((film, idx) => (
          <div key={film.id} className={`flex items-center gap-4 p-4 ${idx !== filtered.length - 1 ? "border-b border-slate-100" : ""} hover:bg-slate-50 transition`}>
            <div className="w-20 h-12 rounded-lg shrink-0 flex items-center justify-center" style={{ background: THUMBNAIL_COLORS[idx % THUMBNAIL_COLORS.length] }}>
            ...
            <div className="flex gap-2 shrink-0">
              <button
                onClick={() => film.url && window.open(film.url, "_blank")}
      ...
      {confirm && (
        <ConfirmModal
          ...
          message={confirm.type === "approve" ? `Are you sure you want to approve "${confirm.film.title}"?` : `Are you sure you want to reject "${confirm.film.title}"?`}
          ...
          {options.map((opt) => (
            <button
              ...
              className={`w-full text-left px-4 py-2.5 text-sm transition cursor-pointer ${
                opt.value === value ? "bg-slate-100 text-slate-900 font-semibold" : "text-slate-700 hover:bg-slate-50"
              }`}
            >
              {opt.label}
            </button>
...
function InitialsAvatar({ name }) {
  ...
  return (
    <div className="w-10 h-10 rounded-full flex items-center justify-center shrink-0 text-white text-xs font-bold shadow-sm" style={{ background: `hsl(${hue}, 55%, 40%)` }}>
      {initials}
    </div>
  );
}
          ...
          <tbody>
            {filtered.map((student, idx) => (
              <tr key={student.id} className={`${idx !== filtered.length - 1 ? "border-b border-slate-100" : ""} hover:bg-slate-50 transition`}>
                ...
                <td className="px-5 py-4">
                  <span className="px-2.5 py-1 rounded-lg bg-slate-100 text-slate-600 text-xs font-semibold">{student.section}</span>
                </td>
                <td className="px-5 py-4">
                ...
                <td className="px-5 py-4">
                  <div className="flex items-center gap-2">
                    ...
                    <button onClick={() => setConfirmRemove(student)} className="p-1.5 rounded-lg text-slate-400 hover:text-red-500 hover:bg-red-50 transition cursor-pointer">
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-md space-y-5">
            ...
            <div className="flex gap-3 pt-2">
              <button onClick={() => setShowModal(false)} className="flex-1 py-2.5 border border-slate-200 rounded-xl text-sm font-semibold text-slate-600 hover:border-[#8B0000] transition cursor-pointer">
                Cancel
              </button>
              <button onClick={handleAddStudent} className="flex-1 py-2.5 bg-[#8B0000] text-white rounded-xl text-sm font-semibold hover:bg-[#6b0000] transition cursor-pointer">
                Add Student
              </button>
            </div>
          </div>
...
function renderRatingStars(rating) {
  ...
  return (
    <div className="flex items-center gap-1">
      {Array.from({ length: fullStars }).map((_, i) => <Star key={`f${i}`} className="w-4 h-4 text-[#FFD700]" fill="currentColor" />)}
      {hasHalf && <StarHalf key="half" className="w-4 h-4 text-[#FFD700]" />}
      {Array.from({ length: emptyStars }).map((_, i) => <Star key={`e${i}`} className="w-4 h-4 text-gray-400" />)}
    </div>
  );
}
      ...
      <div className="relative h-[60vh] w-full overflow-hidden rounded-2xl shadow-xl">
        ...
        <div className="relative z-10 h-full flex items-center px-8 sm:px-10">
          <div className="max-w-2xl space-y-4">
            <span className="inline-flex px-3 py-1 bg-[#8B0000] text-xs text-white rounded-lg">Top 10 – Highlight Film</span>
            ...
            <div className="flex flex-wrap gap-3 mt-4">
              <button className="bg-white text-black px-6 py-3 rounded-lg font-medium hover:bg-gray-100 cursor-pointer shadow-sm">Play Now</button>
              <button className="bg-[#8B0000] text-white px-6 py-3 rounded-lg font-medium hover:bg-[#6b0000] cursor-pointer shadow-sm">More Info</button>
            </div>
          </div>
        </div>
      </div>
  ...
  const TabButton = ({ id, icon: Icon, label }) => (
    <button
      ...
      className={`w-full flex items-center space-x-3 px-4 py-3 rounded-lg transition ${
        activeTab === id ? "bg-[#8B0000] text-white shadow-md" : "text-gray-600 hover:bg-gray-100"
      }`}
    >
      <Icon size={20} />
  ...
  return (
    <div>
      ...
      <div className="pt-6 pb-4">
        <button onClick={logout} className="flex items-center gap-2 px-6 py-3 bg-[#8B0000] text-white rounded-xl font-medium hover:bg-[#6b0000] transition cursor-pointer">
          <LogOut className="w-4 h-4" /> Logout
        </button>
      </div>
    </div>
