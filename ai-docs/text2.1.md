...
- In the sidebar make the icons color white instead of black. make all pressable buttons on cursor.f Instead of Prof 2301947 It should be Welcome, Bry. make the Pending Reviews 8 Approved Films34 Active Submissions 12 Total Students 47 Slightly smaller. make the notification smaller and add small circle background for it. make the profile slighly bigger and add the photo from src/assets/teampics/bryan.png. instead of Pending Review Queue use Pending Films

In the Students add a Default Icon instead if the film icon. Make the students management look like StudentManagement.png but only add Full Name Student Number Course and Section add where they can be search by a button 

- Make 404 Error Page for UB-SINING
